...
class FileService:
    ...
    async def save_file_data(self, file_data: dict) -> dict:
        ...
        file = await self.files.find_one({"_id": result.inserted_id})
    ...
    async def save_bulk_file_data(self, files_data: list[dict], organization_id: str) -> list[dict]:
        ...
        - Updates files that already have _id.
        ...
        for file in files_data:
            ...
            if file.get("_id"):
                try:
                    ...
                    update_data = {k: v for k, v in file.items() if k != "_id"}

                    updated_file = await self.files.find_one_and_update(
                        {"_id": dependencies.try_objectid(file_id)},
                        {"$set": update_data},
                    ...
                    if updated_file:
                        ...
                        new_file = await self.files.find_one({"_id": insert_result.inserted_id})
                        
                        updated_files.append(dependencies.convert_objectid(new_file))
                except Exception as e:
                    print(f"⚠️ Error updating file {file.get('_id')}: {e}")
    ...
    async def delete_file_data(self, file_id: str) -> str:
        ...
        result = await self.files.find_one_and_delete({"_id": ObjectId(file_id)})
